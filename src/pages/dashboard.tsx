...
import { useRoqPlatformUserProfiles, useApplicationCount, useJobOfferCount } from 'lib/roq';

import { JobOfferListPage as ShowCaseList } from 'pages/job-offers';
...
function DashboardPage() {
  ...
  const jobOfferStat = useJobOfferCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Job Offers',
      result: jobOfferStat?.data,
      isLoading: jobOfferStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`job-offers`} passHref legacyBehavior>
                  <Button
