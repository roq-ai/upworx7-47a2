...
import { useRoqClient, useJobOfferFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<JobOfferInterface, unknown>;

interface JobOfferListPageProps {
...
export function JobOfferListPage(props: JobOfferListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useJobOfferFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.job_offer.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: JobOfferInterface) => {
    if (hasAccess('job_offer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/job-offers/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'name', header: 'Name', accessorKey: 'name' },
    { id: 'placed', header: 'Placed', accessorKey: 'placed' },
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="job_offer" />
        </Flex>

        {hasAccess('job_offer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/job-offers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
